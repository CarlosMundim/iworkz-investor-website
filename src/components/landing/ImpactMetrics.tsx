        ...
        return (
          <div key={idx} className="flex flex-col items-center flex-1 min-w-[130px]">
            <span
              ...
              aria-label={String(m.value) + (m.suffix || '')}
            />
            <span className="text-2xl font-semibold text-blue-600 mt-2">{m.suffix}</span>
            <span className="text-lg text-gray-500 mt-3 text-center">{m.label}</span>
          </div>
        );
      })}
    </section>
